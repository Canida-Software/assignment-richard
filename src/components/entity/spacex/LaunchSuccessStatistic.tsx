...
const currentPeriodInMonths = 6;
const currSuccessTitle = `Success in the last ${currentPeriodInMonths} months`;
...
const SuccessStatistic: FC = () => {
  ...
  if (
    ...
    for (let i = 0; i < totalCount; i++) {
      ...
      lastCurrDate.setMonth(lastCurrDate.getMonth() - currentPeriodInMonths);
    ...
    currSuccessRate = `${((currSuccessCount / currLaunchCount) * 100).toFixed(
      2
    )}%`;

    if (currLaunchCount === 0) {
      currSuccessRate = 'No recorded launches. Check back soon!';
    }
  ...
  return (
    <Card title="Success of launches ">
      <Row gutter={16}>
        ...
        <Col span={12}>
          <Statistic
            title={currSuccessTitle}
            value={currSuccessRate}
            loading={isLoading}
          />
        </Col>
      </Row>
